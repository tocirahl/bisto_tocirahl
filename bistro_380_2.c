...
// x**3 curve
#define RAMP_7135  5,5,6,6,7,8,10,12,16,20,25,30,36,42,50,59,68,78,90,103,116,131,148,165,184,204,226,249,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,0
...
uint8_t modegroup = 4;     // which mode group (set above in #defines)
...
uint8_t maxtemp = 88;      // temperature step-down threshold
#endif
    ...
    while(1) {
        if (fast_presses > 0x0f) {  // Config mode
            ...
            if (temp >= maxtemp) {
                ...
                if ((overheat_count > 25) && (actual_level > (RAMP_SIZE/4))) {
                    actual_level --;
                ...
                overheat_count = 0;
                if ((temp < maxtemp - 5) && (actual_level < output)) {
                    actual_level ++;
					_delay_s(); _delay_s(); _delay_s(); _delay_s(); _delay_s(); _delay_s(); 
					_delay_s(); _delay_s(); _delay_s(); _delay_s(); _delay_s(); _delay_s(); 
					_delay_s(); _delay_s(); _delay_s(); _delay_s(); _delay_s(); _delay_s();  // don't step back up too fast and overshoot stable level
